...
class _WebViewScreenState extends State<WebViewScreen>
  ...
  // Thêm biến lưu thông tin game

  Map<String, dynamic>? _gameInfo;
  bool _isOnGamePage = false;
      ...
      if (type == 'login_success') {
        ...
        setState(() {
          ...
          _gameInfo = {
            ...
            'winRate': data['winRate']?.toString() ?? '0%',

            'countdown': data['countdown']?.toString() ?? '',

            'gameImageUrl': data['gameImageUrl']?.toString() ?? '',

            'startTime': data['startTime']?.toString() ?? '',

            'endTime': data['endTime']?.toString() ?? '',

            'vong': data['vong']?.toString() ?? '',
        ...
        debugPrint('🎮 Nhận thông tin game: GameId=${data['gameId']}, GameName=${data['gameName']}, Tỉ lệ=${data['winRate']}, Countdown=${data['countdown']}, StartTime=${data['startTime']}, EndTime=${data['endTime']}, Vòng=${data['vong']}, ImageUrl=${data['gameImageUrl']}');

        // Gửi thông tin game đến overlay nếu overlay đang hoạt động

        _sendGameInfoToOverlay();

      }
      else if (type == 'page_status') {

        // Xử lý trạng thái trang (đang ở game page hay không)

        final isOnGamePage = data['isOnGamePage'] ?? false;

        setState(() {

          _isOnGamePage = isOnGamePage;

        });

        debugPrint('📄 Trạng thái trang: ${isOnGamePage ? "Đang ở trang game" : "Không ở trang game"}');
  ...
  Future<void> _sendGameInfoToOverlay() async {
    ...
    try {
      ...
      if (isActive == true) {
        ...
        debugPrint('✅ Đã gửi game_info_update đến overlay');

      }

    } catch (e) {

      debugPrint('❌ Lỗi khi gửi game_info đến overlay: $e');
  ...
  Future<void> _showOverlayAndSendData() async {
    ...
    if (!_isOnGamePage) {
      ...
      debugPrint(
        ...
        "⚠️ Không hiển thị overlay: Không ở trang game (isOnGamePage=false)",
    ...
    try {
      ...
      // Gửi thông tin game đến overlay

      await _sendGameInfoToOverlay();

      debugPrint("✅ Đã gửi thông tin game đến overlay");
